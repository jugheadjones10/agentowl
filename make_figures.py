...
def make_montezuma_fig1(config: DictConfig) -> None:
    ...
    for idx in range(len(frames)):
        ...
        cropped_frame_image = frame_image.crop(
            (0, height * 0.22, width, height * 0.87)
        )  # Crop 10% from top and bottom
        cropped_frame_image.save(
            f'./images/MontezumaRevenge_fig1_frame_{idx}.png'
        )
...
def make_montezuma_dead(config: DictConfig) -> None:
    ...
    for idx in range(len(frames) - 10, len(frames)):
        ...
        cropped_frame_image.save(
            f'./images/MontezumaRevenge_dead_frame_{idx}.png'
        )
...
def make_montezuma_success(config: DictConfig) -> None:
    ...
    for idx in range(len(frames) - 10, len(frames)):
        ...
        cropped_frame_image.save(
            f'./images/MontezumaRevenge_success_frame_{idx}.png'
        )
...
def make_montezuma_alt_gameplay(config: DictConfig) -> None:
    ...
    for idx in range(len(frames)):
        if idx % 5 == 1:
            ...
            width, height = frame_image.size
            cropped_frame_image = frame_image.crop(
                (0, height * 0.03, width, height * 0.89)
            )  # Crop 10% from top and bottom
...
def make_pong_alt_gameplay(config: DictConfig) -> None:
    ...
    for idx in range(len(frames)):
        if idx % 5 == 1:
            ...
            width, height = frame_image.size
            cropped_frame_image = frame_image.crop(
                (0, height * 0.14, width, height * 0.95)
            )  # Crop 10% from top and bottom
...
def make_pong_planning(config: DictConfig) -> None:
    ...
    for run_idx, actions in enumerate(actions_list):
        ...
        for idx in [68, 73, 78]:
            ...
            width, height = frame_image.size
            cropped_frame_image = frame_image.crop(
                (0, height * 0.15, width, height * 0.93)
            )  # Crop 10% from top and bottom
            cropped_frame_image.save(
                f'./images/Pong_planning_run{run_idx}_frame_{idx}.png'
            )
