...
from .utils import (
    ...
    remove_noncompilable_code_blocks,
)
...
def refine_reward(
    ...
    mission = mission.pop()
    assert all([v['mission'] == mission for v in crt_experiences.values()]), (
        'All experiences should have the same mission.'
    )
    mission = str(mission)
    ...
    wrong_text_experiences = experiences2text_with_wrong_outputs(
        wrong_experiences, code
    )
    if len(crt_experiences) == 0:
        ...
        while (
            ...
            > 6500
            and len(_crt_experiences) > 0
        ):
            _crt_experiences.popitem()
            crt_text_experiences = experiences2text(
                _crt_experiences,
            )
        crt_text_experiences = (
    ...
    while True:
        ...
        result = evaluator(
            code,
            {**crt_experiences, **wrong_experiences},
        )
