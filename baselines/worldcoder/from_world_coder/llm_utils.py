    ...
    def _action(self, prompt, model_args=None):
        ...
        while ct < 10:
            try:
                ...
                ct += 1
        if ct == 10:
            raise Exception('Failed to update local tracker after 10 retries')
        return out
...
def db2fake_completion(db_row):
    ...
    completion = FakeCompletion(
        choices=[FakeChoice(message=message)], usage=usage
    )
    return completion
...
class LLMUsageTracker:
    ...
    def update(self, completion):
        ...
        self.usage['cost'] += (
            2.5e-6 * completion.usage.prompt_tokens
            + 1e-5 * completion.usage.completion_tokens
        )
...
class MinorLLMUsageTracker:
    ...
    def __exit__(self, type, value, traceback):
        ...
        self.usage['completion_tokens'] += (
            self.end['completion_tokens'] - self.start['completion_tokens']
        )
        self.usage['prompt_tokens'] += (
            self.end['prompt_tokens'] - self.start['prompt_tokens']
        )
        self.usage['total_tokens'] += (
            self.end['total_tokens'] - self.start['total_tokens']
        )
