...
class AtariEnv:
    ...
    def __init__(
        ...
        if (
            config.world_model_learner.obj_type == 'beam'
            or config.world_model_learner.obj_type == 'rope'
            or config.rope_mode
            or config.world_model_learner.obj_type == 'disappearingtarpit'
        ):
...
class ImaginedAtariEnv:
    def __init__(
        self, config, world_model, object_tracker: ObjectTracker, renderer
    ):
        self.config = config
        ...
        if config.imagined_atari_env.start_at != 0:
            ...
            for i in range(config.imagined_atari_env.start_at):
                self.init_obj_list, self.init_game_state = atari_env.step(
                    actions[i]
                )
                self.object_tracker.update(self.init_obj_list)
    ...
    def reset(self) -> tuple[ObjListWithMemory, GameState | None]:
        """
        ...
        return ObjListWithMemory(
            self.obj_list, self.memory
...
class PongAltEnv(AtariEnv):
    ...
    def reset(self) -> tuple[ObjListWithMemory, GameState]:
        ...
        self.object_tracker.handle_game_state(
            self.obj_list_no_tags, self.game_state
        )
        self.game_state = self.game_state_tracker.init(self)

        # Render the no-tags object list
        if self.recorder is not None:
            self.recorder.frames.append(
                self.image_renderer.render(self.obj_list_no_tags)
            )
        else:
        ...
        return ObjListWithMemory(
            self.obj_list_no_tags, self.memory
        ), self.game_state
    ...
    def step(self, action: str) -> tuple[ObjListWithMemory, GameState]:
        ...
        self.cum_losses += (
            max(0, -reward1) + max(0, -reward2) + max(0, -reward3)
        )
...
class MontezumaAltEnv(AtariEnv):
    ...
    def __init__(
        ...
        Args:
            ...
            recorder: Optional recorder for capturing gameplay
        """
        if env_name not in ['MontezumaRevengeAlt']:
            raise NotImplementedError
        ...
        for a in self.actions:
            self.env.step(
                self.actions_enum.get_all_possible_actions().index(a)
            )
    ...
    def step(self, action: str) -> tuple[ObjListWithMemory, GameState]:
        ...
        if self.game_state == GameState.RESTART:
            ...
            for a in (
                self.actions + ['LEFT'] * 20 + ['NOOP'] * 20 + self.actions
            ):
                # for a in self.actions + ['NOOP'] * 20:
                self.env3.step(
                    self.actions_enum.get_all_possible_actions().index(a)
    ...
    def _grab_obj_list(self):
        ...
        for idx, o in enumerate(self.env.objects):
            if (
                ...
                or (
                    o.prev_y == 136
                    and o.category.lower() in ['ladder', 'wall']
                )
                or o.category.lower() == 'player'
            ):
        ...
        for idx, o in enumerate(self.env3.objects):
            if (
                ...
                or (o.prev_y > 93 and o.prev_x < 16)
                or o.category.lower() == 'player'
            ):
    ...
    def _grab_action_indices(self, action):
        ...
        flipped_action_index = (
            self.actions_enum.get_all_possible_actions().index(
                action.replace('RIGHT', 'DUMMY')
                .replace('LEFT', 'RIGHT')
                .replace('DUMMY', 'LEFT')
            )
        )
        ...
        if player_pos > 0 and action == 'DOWN' and x >= 125:
            ...
            return (
                flipped_action_index if player_pos == 1 else 0,
                action_index
                if player_pos == 2
                else (
                    0
                    if self.env2.objects[0].x == 129
                    else (4 if self.env2.objects[0].x == 132 else 3)
                ),
                (
                    0
                    if self.env3.objects[0].x == 129
                    else (4 if self.env3.objects[0].x == 132 else 3)
                ),
            )

        return (
            flipped_action_index if player_pos == 0 else 0,
            action_index
            if player_pos == 1
            else (
                0
                if self.env2.objects[0].x == 129
                or self.env2.objects[0].x == 20
                else (4 if self.env2.objects[0].x == 132 else 3)
            ),
            flipped_action_index
            if player_pos == 2
