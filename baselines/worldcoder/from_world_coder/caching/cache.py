...
class _CacheSystem:
    def __init__(self, cache_path=None, seed=0, stochastic=False):
        ...
        if cache_path is None:
            ...
            cache_path = os.path.join(
                curdir, 'cached_data', self.__class__.__name__
            )
    ...
    def __call__(self, *args, nth=None, **kwargs):
        ...
        assert isinstance(cache_id, tuple), (
            f'cache_id must be a tuple, got {type(cache_id)}'
        )
        assert all([len(cid) == 3 for cid in cache_id]), (
            f'cache_id must be a tuple of tuples of length 2, got {cache_id}'
        )
        assert all([isinstance(cid[0], str) for cid in cache_id]), (
            f'cache_id must be a tuple of tuples with first element being a string, got {cache_id}'
        )
        assert all([isinstance(cid[1], str) for cid in cache_id]), (
            f'cache_id must be a tuple of tuples with second element being a string, got {cache_id}'
        )
        ...
        if self.stochastic:
            if nth is None:
                ...
                assert len(self.nth_dict) == 0, (
                    'nth_dict must be empty for deterministic nth'
                )
        else:
            nth = 1
        ...
        # Handle cache
        if (
            unique_cache_id in self.caches
            and self.seed in self.caches[unique_cache_id]['index']
            and nth < len(self.caches[unique_cache_id]['index'][self.seed])
        ):
        ...
        if self.seed in cache['index']:
            ...
            if len(index) < len(cache['response']):
                ...
                index = index + list(
                    np_rng.permutation(
                        list(range(len(index), len(cache['response'])))
                    )
                )
        elif len(cache['response']):
            np_rng = np.random.default_rng(self.seed)
            index = list(
                np_rng.permutation(list(range(len(cache['response']))))
            )
        else:
            index = []
        ...
        # Get response
        if nth > len(index):
            assert nth == len(index) + 1, (
                f'Expected nth to be {len(index) + 1}, got {nth}'
            )
            ...
            updated_cache = False
            print(
                f'Cache hit for {unique_cache_id} with seed {self.seed} and nth {nth}; Cache path: {cache_path}'
            )
    ...
    def _get_cache_path(self, cache_id):
        ...
        for name, cid, value in cache_id:
            ...
            if (
                not os.path.exists(os.path.join(path, 'value.json'))
                and not os.path.exists(os.path.join(path, 'value.dill'))
                and not os.path.exists(os.path.join(path, 'seen_values.json'))
                and not os.path.exists(os.path.join(path, 'seen_values.dill'))
            ):
                ...
                if len(str(seen_values)) < 1e5:
                    try:
                        with open(
                            os.path.join(path, 'seen_values.json'), 'w'
                        ) as f:
                            json.dump(seen_values, f)
                    except:
                        with open(
                            os.path.join(path, 'seen_values.dill'), 'wb'
                        ) as f:
    ...
    def _canonical_value(cls, value):
        if isinstance(value, dict):
            ...
            return tuple(
                sorted([cls._canonical_value(v) for v in value], key=str)
            )
