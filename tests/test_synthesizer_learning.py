...
def test_saved_observation_fixture_matches_run_slice(
    ...
    assert len(
        saved_observation_slice.actions) == learning_config.obs_index_length
    assert len(saved_observation_slice.game_states) == (
        learning_config.obs_index_length + 1)
    assert len(transition_slice) == learning_config.obs_index_length
    assert transition_slice[
        0].input_state is saved_observation_slice.observations[0]
...
def test_no_interact_passive_movement_natural_language_observation_stage(
    ...
    assert stage_repr["llm_call"]["prompt"] == expected_stage_repr["llm_call"][
        "prompt"]
    assert stage_repr["llm_call"]["prompt_count"] == expected_stage_repr[
        "llm_call"]["prompt_count"]
    assert stage_repr["llm_call"]["temperature"] == expected_stage_repr[
        "llm_call"]["temperature"]
    assert stage_repr["llm_call"]["seed"] == expected_stage_repr["llm_call"][
        "seed"]
...
def test_no_interact_passive_movement_synthesizer_end_to_end(
    ...
    interpret_llm_call = _llm_call_repr(case.llm.calls[0],
                                        _interpret_prompt_repr)
    ...
    synthesis_comparison_repr = {
        ...
        "interpret_llm_call": {
            "prompt_count":
            interpret_llm_call["prompt_count"],
            "prompt":
            interpret_llm_call["prompt"],
            "temperature":
            interpret_llm_call["temperature"],
            "seed":
            interpret_llm_call["seed"],
            "output_observations":
            sorted(
                _without_numbering(line)
