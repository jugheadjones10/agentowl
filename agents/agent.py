...
class Agent:
    ...
    def __init__(
        self, config: dict[str, Any], world_learner: WorldModelLearner
    ) -> None:
        """Initialize agent with configuration and world model learner"""
        self.config = config

        renderer = get_human_renderer(config)
        self.atari_env = create_atari_env(
            config, config.task, renderer=renderer
        )
        ...
        self.abstract_planning = (
            True
            if len(self.world_learner.world_model.constraints) > 0
            else False
        )
        ...
        if self.abstract_planning:
            ...
            while True:
                if self.atari_env.n_reset > 10:
                    log.info('NO GOAL AFTER 10 EPISODES')
                    break
                ...
                log.info('All actions taken: ' + str(self.actions_taken))
                ...
                if success:
                    log.info(
                        f'GOT GOAL! ({n_goals_achieved + 1}/{self.config.agent.n_goals_to_achieve})'
                    )
                    ...
                    if (
                        n_goals_achieved
                        >= self.config.agent.n_goals_to_achieve
                    ):
                        break
        else:
            n_goals_achieved = 0
            while True:
                ...
                while len(goal_ids) == 0:
                    ...
                    goal_ids = self._get_goal_ids(cur_obj_list, goal_obj_type)
                log.info(
                    f'Acting randomly for {ct} frames to see a goal object'
                )

                success, cur_obj_list, cur_game_state = (
                    self._try_reach_multiple_goals(
                        goal_ids,
                        cur_obj_list,
                        cur_game_state,
                        n_budget_iterations,
                ...
                if success:
                    ...
                    for _ in range(1):
                        action = np.random.choice(Constants.ACTIONS)
                        cur_obj_list, cur_game_state = self.atari_env.step(
                            action
                        )
                        self.actions_taken.append(action)

                    log.info(
                        'Acting randomly for 1 frames after getting the goal'
                    )
                    ...
                    log.info(
                        'Acting randomly for 10 frames to find a plan to get the goal'
                    )
    ...
    def _get_symbolic_plan(
        ...
        achievables_hsh: dict[tuple[str, int], Any],
    ) -> list[str] | None:
        ...
        return self.search_and_prune_in_graph(
            self._abstract_state(cur_obj_list),
            goal_id,
            skills_hsh,
            achievables_hsh,
        )[0]
    ...
    def _handle_no_plan(
        ...
        Any, Any, dict[tuple[str, str], Any], dict[tuple[str, int], Any], int
    ]:
        ...
        if symbolic_plan is None:
            ...
            while symbolic_plan is None and self.atari_env.n_reset <= 10:
                ...
                if symbolic_plan is None:
                    log.info(
                        "Still can't find with the new graph -- resetting"
                    )
                    cur_obj_list, cur_game_state = self.atari_env.reset()
                    self.actions_taken.append('RESTART')
    ...
    def _execute_plan_step(
        ...
        cur_obj_list, cur_game_state, success, died_by_monster = (
            self.run_low_level(
                cur_obj_list, cur_game_state, n_budget_iterations, target_state
            )
        )
        ...
        if not success:
            ...
            log.info(
                f'Succeed in getting to {target_state} from {prev_state}!'
            )
    ...
    def _try_reach_goal(
        ...
        cur_game_state: Any,
        n_budget_iterations: int,
        achievables_hsh: dict[tuple[str, int], Any] | None,
    ) -> bool:
        """Try to reach final goal from last plan state"""
        log.info(f'Trying to get goal from {final_state}')

        if (
            achievables_hsh is not None
            and (final_state, goal_id) not in achievables_hsh
        ):
            return False
        ...
        if not success:
            ...
            if achievables_hsh is not None and not (
                self.config.agent.ignore_monster and died_by_monster
            ):
    ...
    def _try_reach_multiple_goals(
        ...
        rng = np.random.default_rng(
            (
                int(hashlib.md5(str_seed.encode()).hexdigest(), 16)
    ...
    def run_low_level(
        ...
        while ct < self.config.agent.max_iter:
            ...
            if new_plan is not None:
                ...
                log.info('Did not find plan')
                if (
                    not found_at_least_one_plan
                    and self.abstract_planning
                    and ct < 2
                ):
                    for _ in range(9):
                        cur_obj_list, cur_game_state = self.atari_env.step(
                        ...
                        history_actions.append('NOOP')
                    log.info(
                        'Waited for 10 frames to see if waiting helps since we havent found a plan here'
                    )
                    continue
                else:
                    break
            ...
            for idx, action in enumerate(plan):
                ...
                ) == target_abstract_state and (
                    target_id is not None
                    or self._is_stable_state(cur_obj_list, world_model)
                ):
                    break
            ...
            c = [
                ...
                for data in zip(
                    obj_lists[start_index - 1 :],
                    taken_actions[start_index - 1 :],
                    obj_lists[start_index - 1 + 1 :],
                    game_states[start_index - 1 :],
                    game_states[start_index - 1 + 1 :],
                )
            ]
            cur_c = cur_c + c
            ...
            if (
                self.config.agent.fast_world_update
                and ct % (self.config.agent.max_iter // 10) == 0
            ):
                self.world_learner.update_world_model(
                    cur_c,
                    fast=True,
                    player_only=self.config.agent.update_player_only,
                )
                cur_c = []
            world_model = self.world_learner.world_model
        ...
        if (
            self.config.agent.permanent_world_update
            and (not success)
            and (not (self.config.agent.ignore_monster and died_by_monster))
        ):
            ...
            if found_at_least_one_plan:
                self.world_learner.update_world_model(
                    ...
                    fast=False,
                    player_only=self.config.agent.update_player_only,
                )  # This updates the world model forever
                log.info(
    ...
    def search_and_prune_in_graph(
        ...
        while len(q) > 0:
            ...
            if (abstract_state, goal_id) in achievables_hsh:
                cur_obj_list, plan, _ = achievables_hsh[
                    (abstract_state, goal_id)
                ]
                # If it is no longer consistent with current world model, delete
                if (
                    self.config.agent.prune_bad_edges
                    and not self._on_good_path(
                        cur_obj_list,
                        plan,
                        str([goal_id]),
                        world_model,
            ...
            for abstract_neighbor in skills_mat[abstract_state]:
                if abstract_neighbor not in hsh:
                    cur_obj_list, plan, _ = skills_hsh[
                        (abstract_state, abstract_neighbor)
                    ]
                    # If it is no longer consistent with current world model, delete
                    if (
                        self.config.agent.prune_bad_edges
                        and not self._on_good_path(
                            cur_obj_list, plan, abstract_neighbor, world_model
                        )
                    ):
                        log.info(
    ...
    def build_graph(
        ...
        log.info(
            f'To restart, create a restart_graph.txt file in tmp_params/{folder}'
        )
        ...
        while len(q) > 0 or len(job_q) > 0 or len(running_jobs) > 0:
            ...
            if (
                os.path.exists(restart_file)
                or time.time() - start_time > 3600 * 5
            ):
                if time.time() - start_time > 3600 * 5:
                    log.info('Restarting graph building process after 5 hours')
                else:
                    log.info(
                        'Restarting graph building process after detecting restart file'
                    )
                ...
                for out_id, args in running_jobs:
                    ...
                    if args[0].break_g2:
                        ...
                        job_id = (
                            os.popen(
                                f'sbatch --parsable ./mcts.sub {in_pkl_path} {out_pkl_path}'
                            )
                            .read()
                            .strip()
                        )
                        ...
                        job_id = (
                            os.popen(
                                f'sbatch --parsable run_mcts.py --in_file {in_pkl_path} --out_file {out_pkl_path} > {log_path} 2>&1'
                            )
                            .read()
                            .strip()
                        )
            ...
            running_jobs = self._launch_jobs(
                job_q, running_jobs, folder, max_running_jobs_ct
            )
            ...
            num_iterations = self._log_progress(
                num_iterations, q, job_q, running_jobs, num_job_finished
            )
    ...
    def _initialize_graph(
        self, obj_list: ObjListWithMemory, n_budget_iterations: int, load: bool
    ...
    def _create_jobs_from_queue(
        self,
        q: list[tuple[str, ObjListWithMemory]],
        job_q: list[tuple],
        n_budget_iterations: int,
        goal_obj_type: str,
        no_callables_world_model_path: str,
    ) -> list[tuple]:
    ...
    def _launch_jobs(
        ...
        while job_q and len(running_jobs) <= max_jobs:
            ...
            if args[0].agent.quick_build_graph:
                ...
                plan, concrete_state = self.mcts.search(
                    obj_list,
                    target_abstract_state,
                    self.world_learner.world_model,
                    iterations=n_budget_iterations,
    ...
    def _should_continue_building(
        ...
        work_remaining = (
            len(q) > 0 or len(job_queue) > 0 or len(running_jobs) > 0
        )
    ...
    def _on_good_path(
        ...
        try:
            for action in action_seq:
                ...
                if len(player_objs) != len(
                    old_obj_list.get_objs_by_obj_type('player')
                ) or (
                    len(player_objs) > 0
                    and player_objs[0].history['deleted'][-2] == 1
                ):
                    break
    ...
    def _is_stable_state(
        self, obj_list: ObjListWithMemory, world_model: Any
    ) -> bool:
        ...
        ) == self._abstract_state(obj_list)  # TODO: be careful

    def _abstract_state(
        self, obj_list: Any, target_id: int | None = None
    ) -> str:
        ...
        if target_id is not None:
            ...
            try:
                ...
                return '[-1]'
            return (
                str([target_id]) if player_obj.overlaps(target_obj) else '[-1]'
            )
        else:
    ...
    def _get_possible_abstract_states(self, obj_list: Any) -> list[str]:
        ...
        for rule in constraints.rules:
            ...
            possibilities = []  # for possible object id that can satisfy this constraint
                ...
                for j in range(i + 1, len(all_possibilities)):
                    for id2, obj2 in all_possibilities[j]:
                        if (
                            id1 != id2
                            and check_overlap(
                                (obj1.left_side, obj1.right_side),
                                (obj2.left_side, obj2.right_side),
                            )
                            and check_overlap(
                                (obj1.top_side, obj1.bottom_side),
                                (obj2.top_side, obj2.bottom_side),
                            )
                        ):
                            abstract_state = [-1] * len(all_possibilities)
                            abstract_state[i] = id1
    ...
    def save_edges_as_gif(
        self, edges, filename, repeat_rate=30, world_model=None
    ):
    ...
    def load_graph(self, n_budget_iterations):
        ...
        if os.path.exists(path):
            ...
            log.info(
                f'Loaded n_node_visited={len(hsh)} '
    ...
    def save_graph(
        self, n_budget_iterations, q, hsh, skills_hsh, achievables_hsh
    ):
