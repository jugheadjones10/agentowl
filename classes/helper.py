...
class StateTransitionTriplet:
    def __init__(
        ...
        input_game_state: GameState | None = None,
        output_game_state: GameState | None = None,
        add_ghost=True,
    ):
        """
    ...
    def deepcopy(self) -> 'StateTransitionTriplet':
        ...
        """
        return StateTransitionTriplet(
            self.input_state.deepcopy(),
            copy.deepcopy(self.event),
            self.output_state.deepcopy(),
            self.input_game_state,
            self.output_game_state,
            False,
        )
...
class RandomValues:
    def __init__(
        ...
        logscores: np.ndarray | None = None,
        ...
        self.exp_normalize = (
            torch_exp_normalize if use_torch else exp_normalize
        )
        self.temp = 1
    ...
    def sample(
        self, temp: float = 1, rng: random.Random | None = None
    ) -> float:
        if self.use_torch:
            raise NotImplementedError
        else:
            if rng is not None:
                return rng.choice(
                    self.values, p=self.exp_normalize(self.logscores / temp)
                )
            else:
...
class Interaction:
    ...
    def __repr__(self):
        return (
            f'Interaction -- {repr(self.obj1)} is touching {repr(self.obj2)}'
        )
...
class Obj:
    def __init__(
        ...
        if (
            ...
            self.prev_x, self.prev_y = (
                game_object.prev_xy
                if game_object.prev_xy != (0, 0)
                else (game_object.x, game_object.y)
            )
            self.velocity_x = (
                game_object.dx if game_object.prev_xy != (0, 0) else 0
            )
            self.velocity_y = (
                game_object.dy if game_object.prev_xy != (0, 0) else 0
            )
            self.id = id
    ...
    def falling_time(self):  # TODO: can be optimized
        if 'touch_below' not in self.history or len(
            self.history['touch_below']
        ) != len(self.history['velocity_y']):
            return 0
        falling_ct = 0
        for x, y in zip(
            self.history['velocity_y'][::-1][1:],
    ...
    def old_touches(self, other, shrink_top=0, shrink_bottom=0, shrink_side=0):
        ...
        return pygame.Rect(
            min(self.x - 1 + shrink_side, self.center_x),
            min(self.y - 1 + shrink_top, self.center_y),
            max(1, self.w + 2 - shrink_side),
            max(1, self.h + 2 - shrink_bottom),
        ).colliderect(pygame.Rect(other.x, other.y, other.w, other.h))
    ...
    def touches(self, other, touch_side=-1, touch_percent=0):
        ...
        if touch_side == -1:
            ...
            return pygame.Rect(
                ...
                self.y,
                self.w // 2,
                max(int(self.h * (1 - touch_percent)), 1),
            ).colliderect(
                pygame.Rect(other.x, other.y, other.w, other.h)
            ) and pygame.Rect(
                self.x - 1,
            ...
            return pygame.Rect(
                ...
                self.x + self.w - max(int(self.w * (1 - touch_percent)), 1),
                self.y - 1,
                max(int(self.w * (1 - touch_percent)), 1),
                self.h // 2,
            ).colliderect(pygame.Rect(other.x, other.y, other.w, other.h))
    ...
    def size_propagate(self) -> None:
        if (
            isinstance(self.w_change, RandomValues)
            or isinstance(self.h_change, RandomValues)
            or isinstance(self.x, RandomValues)
            or isinstance(self.y, RandomValues)
        ):
    ...
    def copy(self):
        ...
        )  # Copy history bc dict is a mutable object
        return new_instance
...
class ObjList:
    def __init__(self, objs: Sequence[Obj], no_copy: bool = False) -> None:
        ...
        """
        self.objs = [
            obj.copy() if not no_copy else obj
            for obj in objs
            if obj is not None
        ]
    ...
    def create_object(self, obj_type, x, y, delay_timesteps=-1):
        ...
        for o in self.objs:
            if o.id == -1:
                if not added and (
                    o.prev_x > obj.prev_x
                    or (o.prev_x == obj.prev_x and o.prev_y >= obj.prev_y)
                ):
                    new_objs.append(obj)
                    added = True
            ...
            for j in range(len(self)):
                ...
                for side in range(4):
                    for percent in np.arange(0.1, 1.1, 0.1):
                        if percent > best_touching_percent and self.objs[
                            i
                        ].touches(
                            self.objs[j],
                            touch_side=side,
                            touch_percent=percent,
                        ):
                ...
                if w_id:
                    if best_touching_side == -1:
                        ...
                        touch_side_txt = ['left', 'right', 'top', 'bottom'][
                            best_touching_side
                        ]
...
class ObjTypeInteractionSelector:
    ...
    def __call__(self, interactions):
        return [
            ...
            if ia.obj1.obj_type == self.obj_type
...
def add_noise_to_random_values(
    random_values: RandomValues,
    min_range: int,
    max_range: int,
    log_noise_value: float,
) -> RandomValues:
    """
...
def combine_random_values(
    random_values_lst: Sequence[RandomValues],
    weights: np.ndarray,
    use_torch: bool,
) -> RandomValues:
    """
...
def combine_random_values_torch(
    random_values_lst: Sequence[RandomValues], weights: torch.Tensor
) -> RandomValues:
    """
...
def instantiate_obj_list(
    obj_list_dist: 'ObjList',
    det: bool = False,
    temp: float = 1,
...
def evaluate_logprobs_of_obj_list(
    obj_list_dist: 'ObjList',
    obj_list: 'ObjList',
    by_pos: bool = False,
    ...
    for idx1, idx2 in good_pairs:
        ...
        if obj.deleted != 1:
            if by_pos:
                if isinstance(obj_dist.x, RandomValues):
                    ...
                    logprobs += (
                        0 if obj_dist.x == obj.x else LOG_IMPOSSIBLE_VALUE
                    )
                if isinstance(obj_dist.y, RandomValues):
                    logprobs += obj_dist.y.evaluate_logprobs(obj.y)
                else:
                    logprobs += (
                        0 if obj_dist.y == obj.y else LOG_IMPOSSIBLE_VALUE
                    )
            else:
                ...
                if isinstance(obj_dist.velocity_x, RandomValues):
                    ...
                    logprobs += (
                        0
                        if obj_dist.velocity_x == obj.velocity_x
                        else LOG_IMPOSSIBLE_VALUE
                    )
                if isinstance(obj_dist.velocity_y, RandomValues):
                    ...
                    logprobs += (
                        0
                        if obj_dist.velocity_y == obj.velocity_y
                        else LOG_IMPOSSIBLE_VALUE
                    )
        ...
        if isinstance(obj_dist.deleted, RandomValues):
            ...
            logprobs += (
                0 if obj_dist.deleted == obj.deleted else LOG_IMPOSSIBLE_VALUE
            )
        ...
        if size_change_flag:
            if isinstance(obj_dist.w_change, RandomValues):
                ...
                logprobs += (
                    0
                    if obj_dist.w_change == obj.w_change
                    else LOG_IMPOSSIBLE_VALUE
                )
            ...
            if isinstance(obj_dist.h_change, RandomValues):
                ...
                logprobs += (
                    0
                    if obj_dist.h_change == obj.h_change
                    else LOG_IMPOSSIBLE_VALUE
                )
...
def fill_unset_values_with_uniform(
    obj_list_dist: 'ObjList', size_change_flag: bool = False
) -> None:
    """
    Fills unset values (velocity_x, velocity_y, deleted) with uniform RandomValues.
    """
    all_possible_velocities = np.arange(
        -Constants.MAX_ABS_VELOCITY, Constants.MAX_ABS_VELOCITY + 1
    )
    all_possible_size_changes = np.arange(
        -Constants.MAX_ABS_SIZE_CHANGE, Constants.MAX_ABS_SIZE_CHANGE + 1
    )
    ...
    for obj_dist in obj_list_dist:
        if (
            isinstance(obj_dist.velocity_x, RandomValues)
            or isinstance(obj_dist.velocity_y, RandomValues)
            or isinstance(obj_dist.deleted, RandomValues)
        ):
            found = True
...
def add_noise_to_obj_list_dist(
    obj_list_dist: 'ObjList',
    ...
    log_noise_value_deleted = (
        LOG_NOISE_VALUE_DELETED if not make_uniform else 0
    )
    for obj_dist in obj_list_dist:
        if isinstance(obj_dist.velocity_x, RandomValues):
...
def combine_obj_list_dists(
    obj_list_dists: Sequence['ObjList'],
    ...
    if use_torch:
        indices = [
            idx
            for idx, (obj_list_dist, weight) in enumerate(
                zip(obj_list_dists, weights)
            )
        ...
        obj_list_dists = [
            obj_list_dist
            for idx, obj_list_dist in enumerate(obj_list_dists)
            if idx in indices
        ]
        weights = weights[indices]
    else:
        tmp = [
            (obj_list_dist, weight)
            for obj_list_dist, weight in zip(obj_list_dists, weights)
            if len(obj_list_dist) != 0 and weight != 0
        ]
        if len(tmp) == 0:
            return ObjList([])
        obj_list_dists, weights = (
            list(list(zip(*tmp))[0]),
            np.array(list(zip(*tmp))[1]),
        )

    if padding:
        # Replace old list of obj dists with the new one
        obj_list_dists = pad_obj_list_dists(
            obj_list_dists, size_change_flag=size_change_flag
        )
    ...
    for j in range(len_obj_list):
        ...
        velocity_x = combine_random_values(
            random_values_x_lst, weights, use_torch
        )
        velocity_y = combine_random_values(
            random_values_y_lst, weights, use_torch
        )
        deleted = combine_random_values(
            random_values_deleted_lst, weights, use_torch
        )
...
def match_two_obj_lists_helper(obj_list1, obj_list2, list1_hsh, list2_hsh):
    ...
    for i, obj1 in enumerate(obj_list1):
        ...
        for j, obj2 in enumerate(obj_list2):
            if (
                j not in list2_hsh
                and obj1.obj_type == obj2.obj_type
                and obj1.prev_xy == obj2.prev_xy
            ):  # Try to match objects with same location first
...
def are_two_obj_lists_equal(obj_list1, obj_list2):
    ...
    try:
        obj_list1_sorted = sorted(
            obj_list1.objs, key=lambda obj: (obj.obj_type, obj.x, obj.y)
        )
        obj_list2_sorted = sorted(
            obj_list2.objs, key=lambda obj: (obj.obj_type, obj.x, obj.y)
        )
