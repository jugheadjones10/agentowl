...
def saved_observations(learning_config):
    ...
    identifier = (
        learning_config.task.replace('Alt', '') + learning_config.obs_suffix
    )
...
@pytest.fixture
def no_interact_passive_movement_case(
    transition_slice, learning_config, learning_llm
):
...
def real_llm_enabled(pytestconfig):
    return (
        pytestconfig.getoption('--real-llm')
        or os.getenv('AGENTOWL_REAL_LLM') == '1'
    )
...
def learning_llm(real_llm_enabled, learning_config):
    ...
    try:
        ...
        model_name = (
            'gpt-4o-2024-08-06'
            if learning_config.provider == 'openai'
