...
from .reward_utils import experiences2text, get_func_template
from .utils import (
...
def init_reward(
    ...
    assert len(mission) == 1, (
        f'Experiences should have the same mission, but got {mission}'
    )
    ...
    if verbose_flag:
        print(
            '-' * 20 + 'Guessing initial LambdaReward code: Prompts' + '-' * 20
        )
    ...
    if verbose_flag:
        print(
            '*' * 20
            + 'Guessing initial LambdaReward code: Machine Reply'
            + '*' * 20
        )
        print(gen.content)
        print(cb)
