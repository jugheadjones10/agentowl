...
def get_nl_player_relationships(
    x, player_interactions_selector, constraints=False
):
...
class Synthesizer(ABC):
    ...
    async def _a_get_natural_language_observations(
        self, c, prompt, player_int_selector=False
    ):
        ...
        for x in c:
            ...
            to_be_prompteds.append(
                prompt.format(
                    input=self._prep_interpret_input(
        ...
        outputs = await self.llm.aprompt(
            to_be_prompteds, temperature=0, seed=self.config.seed
        )
        observations = sum(
            [parse_listed_output(output) for output in outputs], []
        )

        res = list(dict.fromkeys(observations))  # remove duplicates
        res.sort()  # order don't matter -- trying to avoid unnecessary LLM calls
        return res
    ...
    def _get_natural_language_effects(self, x):
        ...
        if x not in self.cache_x:
            ...
            if (
                len(input_target_obj_list) == 0
                and len(output_target_obj_list) == 0
            ):
                self.cache_x[x] = []
    ...
    async def _a_prompts_to_codes_helper(
        self, prompts: list[str]
    ) -> list[str]:
        outputs = await self.llm.aprompt(
            prompts, temperature=0, seed=self.config.seed
        )
...
class ActionSynthesizer(Synthesizer):
    ...
    async def a_synthesize(
        ...
        prompts, ns = (
            [
                partial_format(
                    interpret_obj_interact_prompt, obj_type=self.obj_type
                )
            ],
            [4],
        )
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        for i in range(1, min(len(c) + 1, self.max_context_length)):
            ...
            if (
                ...
                if (
                    action == 'NOOP'
                ):  # NOOP is an action action -- let POMDPVelocity handle this
                    return []
                prompts, ns = (
                    [
                        partial_format(
...
class MultiTimestepMomentumSynthesizer(Synthesizer):
            ...
            for i in range(min(len(c), self.max_context_length - 1)):
                ...
                if velocity_dir * objs[0].velocity_x < 0:  # momentum change
                    ...
                    if len(observations_list) != 0:
                        ...
                        res = await self._a_prompts_to_codes_helper(
                            to_be_prompteds
                        )
                        # turn res into a list of tuples with the second element being i
                        res = [(x, i) for x in res]
    ...
    async def _a_get_natural_language_observations(self, c, prompt, mode):
        ...
        to_be_prompteds.append(
            prompt.format(
                input=self._prep_interpret_input(input_target_obj_list, mode),
                effects=list_to_bullets(effects),
            )
        )
    ...
    def _prep_interpret_input(self, obj_list, mode):
        ...
        for obj in obj_list:
            if mode == 'x-axis':
                txt1 += (
                    f'{obj.str_w_id()}'
                    + f' with x-axis velocity = {"%+d" % obj.velocity_x}\n'
                )
...
class MultiTimestepSizeChangeSynthesizer(Synthesizer):
    ...
    def _prep_interpret_input(self, obj_list):
        ...
        for obj in obj_list:
            txt1 += (
                f'{obj.str_w_id()}'
                + f' with width change rate = {"%+d" % obj.w_change}'
                + f' with height change rate = {"%+d" % obj.h_change}\n'
            )
            txt1 += (
                f'{obj.str_w_id()}'
                + f' has its size equal to (w={obj.w},h={obj.h})\n'
            )
...
class MultiTimestepStatusChangeSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        for i in range(2, min(len(c) + 1, self.max_context_length)):
            ...
            if (
                len(objs) > 0
                and objs[0].history['deleted'][-1]
                != objs[0].history['deleted'][-2]
            ):
                prompts, ns = (
                    [
                        partial_format(
                    ...
                    [2],
                )  # TODO: Delete -- there is no prompting
                observations_list = await asyncio.gather(
                    *[
                        self._a_get_natural_language_observations(
                            c[-i:], prompt
...
class MultiTimestepStatusChangeVelocityModeSynthesizer(Synthesizer):
    ...
    def _shorten_list_txt(self, lst):
        ...
        if max_zero_len > 0:
            return (
                str(lst[:max_zero_start])
                + f' + [0] * {max_zero_len} + '
                + str(lst[max_zero_start + max_zero_len :])
            )
        return str(lst)
...
class PassiveMovementSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        prompts, ns = (
            [
                partial_format(
                    interpret_velocity_prompt, obj_type=self.obj_type
                )
            ],
            [6],
        )
...
class PassiveCreationSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        prompts, ns = (
            [
                partial_format(
                    interpret_velocity_creation_prompt, obj_type=self.obj_type
                )
            ],
            [4],
        )
...
class VelocitySynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        prompts, ns = (
            [
                partial_format(
                    interpret_velocity_4_prompt, obj_type=self.obj_type
                )
            ],
            [4],
        )
...
class VelocityTrackingSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        prompts, ns = (
            [
                partial_format(
                    interpret_velocity_3_prompt, obj_type=self.obj_type
                )
            ],
            [2],
        )
    ...
    def _prep_interpret_input(self, obj_list, int_list):
        ...
        int_list = [
            x
            for x in int_list
            if x.obj1.obj_type == 'player'
            and x.obj2.new_velocity_x is not None
            or x.obj2.obj_type == 'player'
            and x.obj1.new_velocity_x is not None
        ]
        ...
        for int in int_list:
            ...
            txt1 += (
                f'{obj.str_w_id()}'
                + f' with new x-axis velocity = {"%+d" % obj.new_velocity_x}'
...
class PlayerInteractionSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        observations_list = await asyncio.gather(
            *[
                self._a_get_natural_language_observations(
                    [x], prompt, player_int_selector=True
                )
                for prompt in prompts
            ]
        )
...
class SnappingSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        to_be_prompteds = [
            explain_event_snapping_prompt.format(
                obj_type=self.obj_type,
...
class ConstraintsSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        to_be_prompteds = [
            explain_event_constraints_prompt.format(
                obj_type=self.obj_type, obs_lst_txt=observation, axis=axis
            )
...
class RestartSynthesizer(Synthesizer):
    ...
    async def a_synthesize(self, c: list[StateTransitionTriplet], **kwargs):
        ...
        (
            ...
            compute_att_txts,
        ) = await self._a_grab_compute_att_callables(x)
        ...
        observations_list = await asyncio.gather(
            *[
                self._a_get_natural_language_observations(
                    c[-1:], prompt, compute_att_callable
                )
                for prompt in prompts
...
"""
    ...
    def _process_func_name(self, txt):
        txt = (
            txt[: len(f'def {self.program_name}')]
            + txt[txt.find('(obj: Obj)') :]
        )
        return txt
    ...
    async def _a_get_natural_language_observations(
        self, c, prompt, compute_att_callable, player_int_selector=False
    ):
        observations = []
        to_be_prompteds = []
        for x in c:
    ...
    def _prep_interpret_input(
        self, compute_att_callable, obj_list, int_list=[]
    ):
        txt1 = ''
        for obj in obj_list:
