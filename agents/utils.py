...
def save_world_model_to_path(world_model):
    ...
    os.makedirs('tmp_world_models', exist_ok=True)
...
def save_frames_as_gif(frames, path='./', filename='gym_animation.gif'):
    ...
    plt.figure(
        figsize=(frames[0].shape[1] / 72.0, frames[0].shape[0] / 72.0), dpi=300
    )
...
def save_frames_as_mp4(
    frames, frameskip=3, path='./', filename='gym_animation.mp4'
):
    # Set up the figure with no padding and lower DPI for better performance
    plt.figure(
        figsize=(frames[0].shape[1] / 72.0, frames[0].shape[0] / 72.0), dpi=60
    ...
    anim = animation.FuncAnimation(
        plt.gcf(), animate, frames=len(frames), interval=50, blit=True
    )  # Enable blitting for faster rendering
    anim.save(
        path + filename,
        writer='ffmpeg',
        fps=60 // frameskip,
