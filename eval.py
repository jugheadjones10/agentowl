...
def obj_lists_partial_match_score(
    predicted_obj_list, actual_obj_list, mode='partial'
):
    moving_obj_types = ['player', 'enemy', 'skull', 'ball']
    predicted_obj_list = ObjList(
        [
            obj
            for obj in predicted_obj_list.objs
            if obj.obj_type in moving_obj_types
        ],
        no_copy=True,
    )
    actual_obj_list = ObjList(
        [
            obj
    ...
    for obj in actual_obj_list:
        ...
        if mode == 'direction':
            ...
            x_dir_score = (
                1
                if (dir_v_x > 0 and pred_v_x > 0)
                or (dir_v_x < 0 and pred_v_x < 0)
                or (dir_v_x == 0 and pred_v_x == 0)
                else 0
            )
            ...
            y_dir_score = (
                1
                if (dir_v_y > 0 and pred_v_y > 0)
                or (dir_v_y < 0 and pred_v_y < 0)
                or (dir_v_y == 0 and pred_v_y == 0)
                else 0
            )
            ...
            score = (
                int(obj.x == predicted_obj_list[best_match_idx].x)
                + int(obj.y == predicted_obj_list[best_match_idx].y)
            ) / 2
        sm += score
...
def make_random_observations(config: DictConfig, length) -> None:
    ...
    """
    recorder = AtariEnvRecorder(config.recording_with_bb)
    ...
    env = create_atari_env(
        ...
        skip_gameover_if_possible=False,
    )
    if config.recording:
...
def grab_transitions(config):
    ...
    if config.eval.set == 'random':
        observations, actions, game_states = make_random_observations(
            config, 1000
        )
    else:
...
def evaluate_world_model(config, world_model):
    ...
    for idx, transition in enumerate(transitions):
        ...
        if config.method == 'worldcoder':
            predicted_state = world_model.sample_next_scene(
                transition.input_state, transition.event
            )
            if config.eval.mode in ['partial', 'direction']:
                correct += obj_lists_partial_match_score(
                    predicted_state,
                    transition.output_state,
                    mode=config.eval.mode,
                )
            else:
                if are_two_obj_lists_equal(
                    predicted_state, transition.output_state
                ):
                    correct += 1
        else:
    ...
    log.info(
        f'Final Accuracy on {config.method} {config.task} {config.eval.mode}: {correct / len(transitions)}'
    )
