...
def evaluate(
    ...
    return {
        ...
        'success_ratio': res['success_ratio'],
    }
...
class InitAction(_Action):
    ...
    def __call__(self):
        indexes = self.local_np_rng.choice(
            len(self.experiences),
            size=min(self.experience_num_per_init, len(self.experiences)),
            replace=False,
        )
...
class RefineTransitAction(_Action):
    ...
    def __call__(
        ...
        log.debug(
            f'{self.__class__.__name__} called {self.called_times} times'
        )
        self.update_param(output['success_flag'])
        return output
    ...
    def main_call(
        ...
        if len(crt_experiences) > self.crt_exp_num_per_refine:
            crt_key_indexes = self.local_np_rng.choice(
                len(crt_experiences),
                size=self.crt_exp_num_per_refine,
                replace=False,
            )
        ...
        wrong_key_indexes = self.local_np_rng.choice(
            len(wrong_experiences), size=1, replace=False
        )
...
def synthesize_transit(
    ...
    log.debug(
        '~' * 20 + os.path.basename(__file__).replace('.py', '') + '~' * 20
    )
    total_cost = 0
    new_cost = 0
    ...
    while total_cost < max_budget:
        ...
        eval_result = output
        if (
            not eval_result['exp_result']['success_flag']
            and len(eval_result['exp_result']['crt_experiences']) > 0
        ):
