...
def get_transit_func(code):
    ...
    if 'transition' in exec_globals and callable(exec_globals['transition']):
        ...
        if len(func_name) == 0:
            ...
            tmp_func_name = {
                k: v for k, v in func_name.items() if 'transit' in k
            }
            if len(tmp_func_name) >= 1:
                func_name = tmp_func_name
        if len(func_name) > 1:
            print(
                f'Warning: Expect only one transition function, but got {len(func_name)}'
            )
    ...
    assert callable(transit_func), (
        f'Expect {func_name} to be callable, but got {transit_func}'
    )
    return compilation_error, func_name, transit_func, exec_globals


def evaluate_transit_code(code, experiences):
    compilation_error, func_name, transit_func, exec_globals = (
    ...
    success_ratio = sum(
        [result['success_flag'] for result in result_list]
    ) / len(result_list)
    pred_state_success_flag = all(
        [result['pred_state_success_flag'] for result in result_list]
    )
    pred_state_success_ratio = sum(
        [result['pred_state_success_flag'] for result in result_list]
    ) / len(result_list)
    crt_experiences = [
        x
        for x, result in zip(experiences, result_list)
...
def eval_transit_per_experience(transit_func, experience, exec_globals):
    ...
    _exec_globals = eval_code(
        code_to_run, exec_globals=locals(), return_exec_globals=True
    )
    ...
    assert not isinstance(_exec_globals, str), (
        f'Failed to run the pre_step of the old state: {_exec_globals}'
    )

    # Run the transit function
    code_to_exec = (
        'pred_new_state = transit_func(copied_old_state, copied_action)'
    )
    exec_globals = eval_code(
        code_to_exec, exec_globals=exec_globals, return_exec_globals=True
    )
    ...
    _exec_globals = eval_code(
        code_to_run, exec_globals=exec_globals, return_exec_globals=True
    )
    assert not isinstance(_exec_globals, str), (
        f'Failed to run the post_step of the new state: {_exec_globals}'
    )

    # Check the success flag
    pred_state_success_flag = are_two_obj_lists_equal(
        pred_new_state, new_state
    )
...
def get_reward_func(code):
    ...
    if 'reward_func' in exec_globals:
        ...
        if len(func_name) == 0:
            ...
            tmp_func_name = {
                k: v for k, v in func_name.items() if 'reward' in k
            }
            if len(tmp_func_name) >= 1:
                func_name = tmp_func_name
        if len(func_name) > 1:
            print(
                f'Warning: Expect only one reward function, but got {len(func_name)}'
            )
            lastest_k = list(func_name.keys())[-1]
            func_name = {lastest_k: func_name[lastest_k]}
    func_name = list(func_name.keys())[0]
    ...
    assert callable(reward_func), (
        f'Expect {func_name} to be callable, but got {reward_func}'
    )
    return compilation_error, func_name, reward_func, exec_globals


def evaluate_reward_code(code, experiences):
    compilation_error, func_name, reward_func, exec_globals = get_reward_func(
        code
    )
    if compilation_error is not None:
        results = {
    ...
    pred_reward_success_flag = all(
        [result['pred_reward_success_flag'] for result in result_list]
    )
    pred_reward_success_ratio = sum(
        [result['pred_reward_success_flag'] for result in result_list]
    ) / len(result_list)
    pred_done_success_flag = all(
        [result['pred_done_success_flag'] for result in result_list]
    )
    pred_done_success_ratio = sum(
        [result['pred_done_success_flag'] for result in result_list]
    ) / len(result_list)
    crt_experiences = {
        k: v
        for (k, v), result in zip(experiences.items(), result_list)
        if result['success_flag']
    }
    wrong_experiences = {
        k: v
...
def eval_reward_per_experience(reward_func, experience, exec_globals):
    assert isinstance(experience, dict), (
        f'Expect experience to be a dict, but got {experience}'
    )
    assert 'state' in experience, (
        f'Expect experience to have key "state", but got {list(experience.keys())}'
    )
    assert 'state_next' in experience, (
        f'Expect experience to have key "state_next", but got {list(experience.keys())}'
    )
    assert isinstance(experience['state'], _State), (
        f'Expect experience["state"] to be an instance of _State, but got {experience["state"]}'
    )
    assert isinstance(experience['state_next'], _State), (
        f'Expect experience["state_next"] to be an instance of _State, but got {experience["state_next"]}'
    )
    assert 'action' in experience, (
        f'Expect experience to have key "action", but got {list(experience.keys())}'
    )
    assert isinstance(experience['action'], _Action), (
        f'Expect experience["action"] to be an instance of _Action, but got {experience["action"]}'
    )
    assert 'reward' in experience, (
        f'Expect experience to have key "reward", but got {list(experience.keys())}'
    )
    assert 'done' in experience, (
        f'Expect experience to have key "done", but got {list(experience.keys())}'
    )
    ...
    if isinstance(exec_globals, str):
        ...
        try:
            ...
            pred_reward_success_flag = False
            compilation_error = (
                f'Failed to compare the reward with the following error: {e}'
            )
