...
from .evaluator import JointEvaluator, SepPlanEvaluator
...
def evaluate(
    ...
    for mission in key_missions:
        if mission not in reward_code:
            reward_code[mission] = guess_reward(
                reward_code, mission, llm, code_example_num=3
            )
...
class _Action:
    ...
    def sample(
        ...
        if self.strategy == 'bandits':
            ...
            return self.value + self.np_rng.normal(
                0, 1e-5
            )  # Randomness to break ties, for better exploration
        else:
            raise NotImplementedError
...
class InitAction(_Action):
    ...
    def __call__(self):
        ...
        key_indexes = self.local_np_rng.choice(
            len(keys),
            size=min(self.experience_num_per_init, len(keys)),
            replace=False,
        )
        ...
        mission = sorted(list(missions), key=str)[
            self.local_np_rng.choice(len(missions))
        ]
        experiences = {
            k: v
            for k, v in self.experiences.items()
            if str(v['mission']) == str(mission)
        }
...
class RefineTransitAction(_Action):
    def __init__(
        ...
        assert parent is not None, (
            f'{self.__class__.__name__} must have a parent'
        )
    ...
    def hashable(
        ...
        return (
            ...
            self.init_transit_code,
            hashdict(self.init_reward_code),
            tuple(
                sorted(
    ...
    def main_call(
        ...
        assert len(crt_experiences) > 0, 'len(crt_experiences) == 0'
        assert len(wrong_experiences) > 0, 'len(wrong_experiences) == 0'
        if len(crt_experiences) > self.crt_exp_num_per_refine:
            crt_keys = list(sorted(crt_experiences.keys()))
            crt_key_indexes = self.local_np_rng.choice(
                len(crt_keys), size=self.crt_exp_num_per_refine, replace=False
            )
            crt_experiences = {
                crt_keys[i]: crt_experiences[crt_keys[i]]
                for i in crt_key_indexes
            }
        else:
            crt_experiences = crt_experiences
        wrong_keys = list(sorted(wrong_experiences.keys()))
        wrong_key_indexes = self.local_np_rng.choice(
            len(wrong_keys), size=1, replace=False
        )
        wrong_experiences = {
            wrong_keys[i]: wrong_experiences[wrong_keys[i]]
            for i in wrong_key_indexes
        }
...
class RefineRewardAction(RefineTransitAction):
    def main_call(
        ...
        assert len(wrong_experiences) > 0, 'len(wrong_experiences) == 0'
        missions_with_crt_exp = {
            str(v['mission']) for v in crt_experiences.values()
        }
        missions = {
            str(v['mission'])
            for v in wrong_experiences.values()
            if str(v['mission']) in missions_with_crt_exp
        }
        missions = missions.intersection(self.key_missions)
        if len(missions) == 0:
            missions = {str(v['mission']) for v in wrong_experiences.values()}
            missions = missions.intersection(self.key_missions)
        assert len(missions) > 0, 'len(missions) == 0'
        mission = sorted(list(missions), key=str)[
            self.local_np_rng.choice(len(missions))
        ]
        crt_experiences = {
            k: v
            for k, v in crt_experiences.items()
            if str(v['mission']) == mission
        }
        wrong_experiences = {
            k: v
            for k, v in wrong_experiences.items()
            if str(v['mission']) == mission
        }
        # assert len(crt_experiences) > 0, f'len(crt_experiences) == 0'
        assert len(wrong_experiences) > 0, 'len(wrong_experiences) == 0'
        if len(crt_experiences) > self.crt_exp_num_per_refine:
            ...
            crt_key_indexes = self.local_np_rng.choice(
                len(crt_keys),
                size=min(self.crt_exp_num_per_refine, len(crt_keys)),
                replace=False,
            )
            crt_experiences = {
                crt_keys[i]: crt_experiences[crt_keys[i]]
...
def synthesis(
    ...
    exp_id = hashlib.md5(
        str(
            (
                ...
                str(sorted(env_metadata.items())),
                str(sorted(llm.default_args.items())),
                plan_obj_flag,
                strategy,
                bandits_C,
            )
    ...
    if 'actions' not in logs:
        ...
        if init_transit_code is not None and init_reward_code is not None:
            ...
            best_success_ratio = (
                eval_result['exp_result']['success_ratio'],
                eval_result['plan_result']['success_ratio']
                if eval_result['plan_result']['success_ratio'] is not None
                else 0,
            )
            best_outputs.append(eval_result)

            code2node_idx[(init_transit_code, hashdict(init_reward_code))] = (
                node_idx
            )
            node_idx += 1
            exp_success_ratio_list.append(
                eval_result['exp_result']['success_ratio']
            )
            ...
            if True and not eval_result['exp_result']['reward_success_flag']:
                actions.append(
                    RefineRewardAction(
                        len(actions),
                        init_transit_code,
            ...
            if (
                plan_obj_flag
                and (
                    eval_result['plan_result'] is not None
                    and not eval_result['plan_result']['success_flag']
                )
                and eval_result['exp_result']['success_flag']
            ):
                actions.append(
                    RefineToPlanTRAction(
                        len(actions),
                        init_transit_code,
                        init_reward_code,
                        experiences,
    ...
    for step in range(logs['step'], max_step_num):
        ...
        print(
            'total costs:',
            {k: np.sum(v) for k, v in total_costs.items() if len(v) > 0},
        )
        print(
            'total new costs:',
            {k: np.sum(v) for k, v in total_new_costs.items() if len(v) > 0},
        )
        ...
        print(
            'exp_result:',
            {
                ...
                if itemnum(v) < 50 and len(str(v)) < 100
            },
        )
        ...
        if (
            output['exp_result']['success_ratio'],
            output['plan_result']['success_ratio']
            if output['plan_result']['success_ratio'] is not None
            else 0,
        ) > best_success_ratio:
            best_success_ratio = (
                output['exp_result']['success_ratio'],
        ...
        if (
            output['exp_result']['transit_success_flag']
            and output['transit_code'] not in perfect_transit_code
        ):
            print(
                'found new perfect transit code, do cross-product-evaluation'
            )
            ...
            for prc in sorted(list(perfect_reward_code)):
                ...
                if (
                    tmp_output['exp_result']['success_ratio'],
                    tmp_output['plan_result']['success_ratio']
                    if tmp_output['plan_result']['success_ratio'] is not None
                    else 0,
                ) > best_success_ratio:
                    ...
                    best_success_ratio = (
                        tmp_output['exp_result']['success_ratio'],
                        tmp_output['plan_result']['success_ratio']
                        if tmp_output['plan_result']['success_ratio']
                        is not None
                        else 0,
                    )
                    best_outputs = [
        ...
        if (
            output['exp_result']['reward_success_flag']
            and hashdict(output['reward_code']) not in perfect_reward_code
        ):
        ...
        eval_result = output
        if (
            not eval_result['exp_result']['transit_success_flag']
            and len(eval_result['exp_result']['crt_transit_experiences']) > 0
        ):
        ...
        if (
            True
            and not eval_result['exp_result']['reward_success_flag']
            and len(eval_result['exp_result']['crt_reward_experiences']) > 0
        ):
        ...
        if (
            ...
            assert new_action == new_action, (
                'RefineToPlanAction must be hashable'
            )
            if new_action not in visited_actions:
                actions.append(new_action)
                visited_actions.add(new_action)
        ...
        if action.parent is None:
            code2node_idx[
                (output['transit_code'], hashdict(output['reward_code']))
            ] = node_idx
            node_idx += 1
            exp_success_ratio_list.append(
                output['exp_result']['success_ratio']
        ...
        if (
            step % 10 == 0
            or step == max_step_num - 1
            or output['success_flag']
        ):
            with open(cache_path, 'wb') as f:
                dill.dump(pickable(logs), f)
        save_time = time.time() - start_time - logging_time
        print(
            f'logging_time: {logging_time:.2f}s, save_time: {save_time:.2f}s'
        )

        with open(
            os.path.join(
                cache_dir,
                f'step{step + 1}_logs_{output["exp_result"]["success_ratio"] * 100:.1f}_{action.__class__.__name__}_{action.eval_result["success_ratio"] * 100 if "eval_result" in dir(action) else 0:.1f}.pkl',
            ),
            'wb',
        ) as f:
    ...
    print(
        f'After {step + 1} steps, best success ratio is {best_success_ratio} with {len(best_outputs)} possible solutions. Randomly choose one.'
    )
