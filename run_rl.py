...
from classes.envs.renderer import get_empty_renderer
...
def add_gym_wrapper(
    config: DictConfig,
    atari_env: WrappedOCAtariEnv,
    truncate_length: int = 36000,
) -> GymAtariEnv:
    ...
    env = GymAtariEnv(
        atari_env, actions, config, truncate_length=truncate_length
    )
...
def create_env(
    config: DictConfig, rank: int, seed: int = 0, eval_env: bool = False
) -> Callable:
    ...
    def _init() -> gym.Env:
        ...
        if config.use_world_as_env:
            ...
            env = create_wrapped_ocatari_env(
                config,
                config.task,
                no_visual=True,
        ...
        if config.use_world_as_env:
            ...
            truncate_length = (
                36000  # Consider lowering this for World Environments
            )
    ...
    def _on_step(self) -> bool:
        ...
        for info in infos:
            if 'episode' in info:
                ...
                # Log to Tensorboard
                self.logger.record(
                    'rollout/ep_rew_mean', np.mean(self.rollout_rewards)
                )
                self.logger.record(
                    'rollout/ep_len_mean', np.mean(self.rollout_lengths)
                )
                self.logger.record(
                    'rollout/ep_rew_std', np.std(self.rollout_rewards)
                )
                self.logger.record(
                    'rollout/ep_len_std', np.std(self.rollout_lengths)
                )
...
def setup_callbacks(
    n_envs: int, eval_env: gym.Env, ckpt_path: Path, config=None
) -> CallbackList:
...
def create_model(
    env: gym.Env, config, latest_ckpt: str = None, load_ckpt: str = None
) -> PPO:
    ...
    policy_kwargs = dict(
        net_arch=dict(
            pi=config.ppo.policy_net_arch, vf=config.ppo.policy_net_arch
        )
    )
...
def main(config: DictConfig):
    ...
    if config.pretrained_model_file:
        ...
        if ckpt_path.is_dir():
            ...
            model = create_model(
                vec_env, config, load_ckpt=config.pretrained_model_file
            )
