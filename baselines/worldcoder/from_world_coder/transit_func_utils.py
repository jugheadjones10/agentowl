...
def experiences2text(c):
    ...
    text_experiences = [
        ...
        f'The action "{action}" transforms the state from\n```\n{old_state}\n```\nto\n```\n{new_state}\n```\n'.strip()
        for old_state, action, new_state in zip(
            old_state_list,
            action_list,
            new_state_list,
        )
...
def experiences2text_with_wrong_outputs(c, code):
    ...
    pred_new_state_list = [
        res['pred_new_state'].get_str_w_ints_w_touching()
        if res['pred_new_state'] is not None
        else None
    ...
    assert (
        len(old_state_list)
        == len(new_state_list)
        == len(pred_new_state_list)
        == len(action_list)
    ), (
        f'Expect all lists to have the same length, but got {len(old_state_list)}, {len(new_state_list)}, {len(pred_new_state_list)}, {len(action_list)}'
    )
    ...
    text_experiences = [
        ...
        _exp2text_wrong(
            old_state, action, new_state, pred_new_state, compilation_error
        )
        for old_state, action, new_state, pred_new_state, compilation_error in zip(
            old_state_list,
            action_list,
            new_state_list,
            pred_new_state_list,
...
def _exp2text_wrong_with_diff(
    old_state, action, new_state, diff, pred_new_state, compilation_error
):
    if compilation_error is not None:
        return f'The action "{action}" should transform the state from\n```\n{old_state}\n```\nto\n```\n{new_state}\n```\nThe difference is\n"""\n{diff}\n"""\nHowever, the implementation fails to compile with error\n```\n{compilation_error[: compilation_error.rfind("Printed outputs:")].strip()}\n```\n'.strip()
    elif pred_new_state != new_state:
