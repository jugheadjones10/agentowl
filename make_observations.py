...
def make_observations(
    config: DictConfig, actions: list[str], name: str
) -> None:
    """
    ...
    """
    recorder = (
        AtariEnvRecorder(config.recording_with_bb)
        if config.recording
        else None
    )
    ...
    with open(f'saved_data/{name}.pickle', 'wb') as f:
        pickle.dump(
            (
                observations[max(0, config.obs_index) :],
                ret_actions[max(0, config.obs_index) :],
                game_states[max(0, config.obs_index) :],
            ),
            f,
        )
...
def main(config: DictConfig) -> None:
    ...
    if config.manual_control:
        ...
        make_observations(
            config, montezuma_actions_basic17, f'obs_{config.task}_basic17'
        )
    ...
    #                       f'obs_{config.task}_works')
        ...
        make_observations(
            config, pitfall_actions_basics1, f'obs_{config.task}_basic1'
        )
    elif config.task == 'Breakout':
        make_observations(
            config, breakout_actions_basic1, f'obs_{config.task}_basic1'
        )
    elif config.task == 'Pong':
        make_observations(
            config, pong_actions_basic2, f'obs_{config.task}_basic2'
        )
