...
def experiences2text(experiences):
    ...
    difference_list = [
        experiences[k]['state_next'] - experiences[k]['state'] for k in keys
    ]
    text_experiences = [
        f'The action "{action}" transforms the state from\n```\n{old_state}\n```\nto\n```\n{new_state}\n```\nThe difference is\n"""\n{diff}\n"""\n, the returned reward is ` {reward} ` and the returned done is ` {done} `'.strip()
        for old_state, action, new_state, reward, done, diff in zip(
            old_state_list,
            action_list,
            new_state_list,
            reward_list,
...
def experiences2text_with_wrong_outputs(experiences, code):
    ...
    pred_reward_list = [
        res['pred_reward'] if res['pred_reward'] is not None else None
        for res in result_list
    ]
    pred_done_list = [
        res['pred_done'] if res['pred_done'] is not None else None
        for res in result_list
    ]
    ...
    assert (
        ...
        == len(reward_list)
        == len(done_list)
        == len(pred_reward_list)
        == len(pred_done_list)
    ), (
        f'Expect all lists to have the same length, but got {len(old_state_list)}, {len(new_state_list)}, {len(difference_list)}, {len(action_list)}, {len(reward_list)}, {len(done_list)}, {len(pred_reward_list)}, {len(pred_done_list)}'
    )
    ...
    text_experiences = [
        ...
        for old_state, action, new_state, reward, done, diff, pred_reward, pred_done, compilation_error in zip(
            old_state_list,
            action_list,
            new_state_list,
            reward_list,
...
def _exp2text_wrong(
    ...
    if compilation_error is not None:
        ...
        print(
            'Evaluating the implementation with the correct reward and done, but got correct implementation'
        )
        return f'The action "{action}" should transform the state from\n```\n{old_state}\n```\nto\n```\n{new_state}\n```\nThe difference is\n"""\n{diff}\n"""\n, the returned reward should be ` {reward} ` and the returned done should be ` {done} `.'.strip()
        raise ValueError(
            'Expect the implementation to be wrong, but got correct implementation'
        )
